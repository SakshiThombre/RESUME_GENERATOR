
<?php
  require("dbcon.php");
  session_start();
?>

<?php    
        require("dbcon.php");
        $id=$_SESSION["id"];
        
        $name=$_SESSION["uname"];
        $sql="select * from studphoto where id='$id' and name='$name'";
        $res=mysqli_query($link,$sql);
        if(mysqli_num_rows($res)>0)
        {
        if($row=mysqli_fetch_array($res))
        {
            ?>
            <?php $img=$row[1]; ?>
            <?php
        }
        }
?>
<!DOCTYPE html>
<html lang="">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Title Page</title>

        <!-- Bootstrap CSS -->
        <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet">

        <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
        <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
        <!--[if lt IE 9]>
            <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.3/html5shiv.js"></script>
            <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
        <![endif]-->
        <style>
            .c1{
                height:150px;
                background-color:#F2C1AD;
                margin-top:70px;
            }
            .c2{
                height:1000px;
                background-color:#00A86B;
                position:relative;
                margin:-300px 0px 0px 70px;
            }
            .c3{
                width:170px;
                height:170px;
                border-radius:360px;
                background-size:100% 100%;
                border:2px solid #fff;
                margin:130px 0px 0px 0px;
                background-image:url(<?php echo $img ?>);
            }
           .c4{
               padding:25px 0px 0px 0px;
             }
             #b1{
               font-size:35px;
             }
             #s1{
                 font-size:16px;
             }
             .c5{
                padding-top:30px;
             }
             .c7{
                height:400px;
                background-color:black;
             }
             .m1{
                padding-top:0px;
             }
             .icon{
                color:#00A86B;
                font-weight:bold;

             }
             @media screen and (max-width:400px){
                .c4{
                    z-index:2;
                    padding-left:70px;
                    padding-top:60px;
                }
                .c3{
                width:80px;
                height:80px;
                }
                #c5{
                padding-top:100px;
             }
             }
             
        </style>
    </head>
    <body>
          
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 c1">
                          
            <div class="col-xs-5 col-sm-5 col-md-5 col-lg-5">
            
            </div>
                
                <div class="col-xs-12 col-sm-12 col-md-5 col-lg-5  c4">
                            <?php
                                    $id=$_SESSION["id"];
                                    $sql="select * from tblstudent where id='$id'";
                                    $res=mysqli_query($link,$sql);
                                    if(mysqli_num_rows($res)>0)
                                    {
                                        if($row=mysqli_fetch_array($res))
                                        {
                                        $pro=$row["profession"];
                                        }
                                    }
                                ?>
                                <b id="b1"><?php  echo $_SESSION["uname"]."<br>";  ?></b>
                                <span id="s1"> <?php echo $pro; ?></span>
                </div>
                
                          
        </div>
          
          <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 c2" style="padding:0px">
               
             <img src="<?php  echo $img; ?>" class="img-responsive c3" alt="Image">
              
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 c5" id="c5">
                   <h4><b>PROFILE</b></h4>
                        <?php
                                $id=$_SESSION["id"];
                                $sql="select * from tblobjective where id='$id'";
                                $res=mysqli_query($link,$sql);
                                if(mysqli_num_rows($res)>0)
                                {
                                    if($row=mysqli_fetch_array($res))
                                    {
                                    $obj=$row["obj"];
                                    }
                                }
                            ?>
                            <p id="p1">
                                <?php echo $obj;  ?>
                            </p>
              </div>

              
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 c5">
                  <h4><b>CONTACT ME</b></h4>
                         <?php
                                    $id=$_SESSION["id"];
                                $sql="select * from tblstudent where id='$id'";
                                    $res=mysqli_query($link,$sql);
                                    if(mysqli_num_rows($res)>0)
                                    {
                                        if($row=mysqli_fetch_array($res))
                                        {
                                            $mob=$row["3"];
                                            $email=$row["2"];
                                        }
                                    }
                                ?>

                                <?php
                                $id=$_SESSION["id"];
                                $sql="select * from tblpersonal where id='$id'";
                                $res=mysqli_query($link,$sql);
                                if(mysqli_num_rows($res)>0)
                                {
                                    if($row=mysqli_fetch_array($res))
                                    {
                                        $pin=$row["pincode"];
                                        $add=$row["address"];
                                        $city=$row["city"];
                                        $state=$row["state"];
                                    }
                                }
                            ?>

       
                            <span class="glyphicon glyphicon-earphone e1" aria-hidden="true">
                                <span id="p2"><?php echo $mob; ?></span>
                            </span> <br>
                            <span class="glyphicon glyphicon-envelope e1" aria-hidden="true">
                                <span id="p2"> <?php echo $email; ?></span>
                            </span> <br>
                            
                                <span class="glyphicon glyphicon-map-marker e1" aria-hidden="true">
                                    <span id="p2"> <?php echo $pin."-".$city.",".$state; ?></span>
                            </span> <br>


                            
                        <?php
                            $sql="select *from tbllinks where id='$id'";
                            $res=mysqli_query($link,$sql);
                            if(mysqli_num_rows($res)>0)
                            {
                            
                                while($row=mysqli_fetch_array($res))
                                {       ?>                                   
                                            <span class="glyphicon glyphicon-link e1" aria-hidden="true">
                                            <span id="p2"> <?php echo $row["media"]."(".$row["links"].")";?></span>
                                        </span> <br>
                                        <?php
                                }
                            }
                        ?>

                          
              </div>
              
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 c5">
                            <h4><b>TECHNICALSKILLS</b></h4>
                                
                                    <?php
                                        $sql="select *from tblhard where id='$id'";
                                        $res=mysqli_query($link,$sql);
                                            if(mysqli_num_rows($res)>0)
                                        {
                                        
                                            while($row=mysqli_fetch_array($res))
                                            {                                          
                                                    echo $row["2"]."<br>";
                                            }
                                        }
                                    ?>
                                
                </div>
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 c5">
                <h4><b>HOBBIES</b></h4>
                            
                                <?php
                                    $sql="select *from tblhobbies where id='$id'";
                                    $res=mysqli_query($link,$sql);
                                        if(mysqli_num_rows($res)>0)
                                    {
                                    
                                        while($row=mysqli_fetch_array($res))
                                        {                                          
                                                echo $row["2"]."<br>";
                                        }
                                    }
                                ?>
                         
                   </div>
              </div>

               
                

              
              
               
          </div>
          
         
          
          <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 c6">
                   
                   <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 m1">
                        <ul><h4><b>EDUCATION</b></h4>
                        </ul>
                        <?php
                            $sql="select *from tbleducationo where id='$id'";
                            $res=mysqli_query($link,$sql);
                            if(mysqli_num_rows($res)>0)
                            {
                                while($row=mysqli_fetch_array($res))
                                {
                                    ?>
                                    <ul>
                                        
                                        <li><?php echo"<b>".$row[2]."(".$row[4].")<br></b>"; ?>
                                        <?php echo"".$row[3]."-".$row[5]."<br>"; ?></li>
                                </ul>

                                    <?php
                                }
                            }
                        ?>
                   </div>
                   
                   <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 m1">
                        <ul><h4><b>EXPERIENCE</b></h4></ul>
                            <UL>
                                <?php
                                    $sql="select *from tbLexperience where id='$id'";
                                    $res=mysqli_query($link,$sql);
                                    if(mysqli_num_rows($res)>0)
                                    {
                                        while($row=mysqli_fetch_array($res))
                                        {?>
                                           
                                           <span class="glyphicon glyphicon-menu-right icon" aria-hidden="true">                                              
                                           </span>   
                                           <?php echo $row["experience"]."<br><br>"; ?>                                                                           
                                        <?php
                                        }
                                    }
                                ?>
                            </UL>
                   </div>
                   
                   
                   <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 m1">
                            <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                                    <ul> <h4><b>SOFTSKILLS</b></h4></ul>
                                        <ul>
                                            <?php
                                                $sql="select *from tblsoft where id='$id'";
                                                $res=mysqli_query($link,$sql);
                                                    if(mysqli_num_rows($res)>0)
                                                {
                                                
                                                    while($row=mysqli_fetch_array($res))
                                                    {                                          
                                                            echo"<li>".$row["2"]."</li>";
                                                    }
                                                }
                                            ?>
                                        </ul>
                            </div>

                            <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                                    <ul> <h4><b>LANGUAGES</b></h4></ul>
                                        <ul>
                                            <?php
                                                $sql="select *from tblang where id='$id'";
                                                $res=mysqli_query($link,$sql);
                                                    if(mysqli_num_rows($res)>0)
                                                {
                                                
                                                    while($row=mysqli_fetch_array($res))
                                                    {                                          
                                                            echo"<li>".$row["2"]."</li>";
                                                    }
                                                }
                                            ?>
                                        </ul>
                            </div>
                   </div>
                   
                   
                   
          </div>
          
          
          
        <!-- jQuery -->
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
        <!-- Bootstrap JavaScript -->
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
    </body>
</html>
